...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (user.role === "admin") {
      ...
      if (!employeeRecordId) {
        return json(res, 400, { error: "Employee is required." });
      }
    ...
    // EMPLOYEE changes own password when required
    if (user.role === "employee") {
      ...
      if (!employee || !employee.active || !employee.time_clock_enabled) {
        return json(res, 403, {
          error: "Employee is not authorized to use Time Clock."
        });
      }

      if (!employee.must_change_password) {
        return json(res, 409, {
          error: "A password change is not currently required."
        });
      }
